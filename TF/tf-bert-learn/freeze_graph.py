...
def init_predict_var(path):
    ...
    if os.path.exists(label2id_file):
        ...
        print("con\'t found %s" % label2id_file)
...
def optimize_class_model(args, logger=None):
    if not logger:
        ...
        pass
    try:
        # 如果PB文件存在，则返回PB文件的路径， 否则将模型转化为PB文件，并且返回存储PB文件的路径
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='directory of pretrained BERT model,default =model-dir'
    )
    parser.add_argument(
        '-max_seq_len',
        type=int,
        default=128,
        help='maximum length of a sequnce ,default:128'
    )
    parser.add_argument(
        '-num_labels',
        type=int,
